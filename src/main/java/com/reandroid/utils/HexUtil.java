...
public class HexUtil {
    ...
    public static String toHexString(byte[] bytes, int start, int length) {
        length = NumbersUtil.min(length, bytes.length - start);
    ...
    public static String toHexStringUpper(byte[] bytes) {
        return toHexStringUpper(bytes, 0, bytes.length);
    }
    public static String toHexStringUpper(byte[] bytes, int length) {
        return toHexStringUpper(bytes, 0, length);
    }
    public static String toHexStringUpper(byte[] bytes, int start, int length) {
        length = NumbersUtil.min(length, bytes.length - start);
        StringBuilder builder = new StringBuilder(length * 2);
        length = start + length;
        for (int i = start; i < length; i++) {
            int b = bytes[i];
            builder.append(toHexCharUpper((b >> 4) & 0xf));
            builder.append(toHexCharUpper(b & 0xf));
        }
        return builder.toString();
    }
    ...
    public static char toHexCharUpper(int i) {
        if (i >= 0) {
            if (i < 10) {
                i = i + '0';
                return (char) i;
