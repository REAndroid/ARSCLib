...
public class MethodKey extends NamedTypeKey implements MemberKey {

    public MethodKey(TypeKey declaring, StringKey name, ProtoKey proto) {
        super(declaring, name, proto);
    }
    private MethodKey(TypeKey declaring, String name, ProtoKey proto){
    ...
    public int getRegister(int index) {
        return getType().getRegister(index);
    }
    public int getParameterIndex(int index) {
        return getType().getParameterIndex(index);
    }

    @Override
    public MethodKey changeDeclaring(TypeKey declaring) {
        return (MethodKey) super.changeDeclaring(declaring);
    }
    @Override
    public MethodKey changeName(String name) {
        return (MethodKey) super.changeName(name);
    }
    @Override
    ...
    public MethodKey changeParameters(TypeListKey parameters) {
        return changeType(getType().changeParameters(parameters));
    }
    public MethodKey changeParameter(int index, TypeKey parameter) {
        return changeType(getType().changeParameter(index, parameter));
    }
    public MethodKey changeProto(ProtoKey protoKey) {
        return changeType(protoKey);
    }
    @Override
    public MethodKey changeType(TypeDescriptorKey protoKey) {
        return (MethodKey) super.changeType(protoKey);
    }
    @Override
    protected NamedTypeKey newInstance(TypeKey declaring, StringKey nameKey, TypeDescriptorKey protoKey) {
        if (declaring == null || !(protoKey instanceof ProtoKey)) {
            return super.newInstance(declaring, nameKey, protoKey);
        }
        return MethodKey.create(declaring, nameKey, (ProtoKey) protoKey);
    }
    public MethodKey changeReturnType(TypeKey typeKey) {
        return changeType(getType().changeReturnType(typeKey));
    }
    public MethodKey removeParameter(int index) {
        return changeType(getType().removeParameter(index));
    }
    public ProtoKey getProto() {
        return getType();
    }
    @Override
    public ProtoKey getType() {
        return (ProtoKey) super.getType();
    }
    ...
    public int getParameterRegistersCount(){
        return getType().getParameterRegistersCount();
    }
    ...
    public boolean equalsIgnoreDeclaring(MethodKey other) {
        ...
        return ObjectsUtil.equals(getNameKey(), other.getNameKey()) &&
                ObjectsUtil.equals(getType(), other.getType());
    }
    public boolean equalsIgnoreReturnType(MethodKey other){
        ...
        return getType().equalsParameters(other.getType());
    }
    ...
    public boolean equalsIgnoreName(MethodKey other) {
        ...
        return ObjectsUtil.equals(getType(), other.getType());
    }
    ...
    public boolean equalsReturnType(TypeKey returnType) {
        return getType().equalsReturnType(returnType);
    }
    public boolean equalsReturnType(MethodKey other){
        ...
        return getType().equalsReturnType(other.getType());
    }
    ...
    public static MethodKey convert(Method method) {
        ...
        TypeKey returnType = TypeKey.convert(method.getGenericReturnType());
        Type[] parameters = method.getGenericParameterTypes();
        int length = parameters.length;
        TypeKey[] typeKeys = new TypeKey[length];
        for (int i = 0; i < length; i++) {
            typeKeys[i] = TypeKey.convert(parameters[i]);
        }
        ProtoKey protoKey = ProtoKey.create(returnType, typeKeys);
        return create(declaring, StringKey.create(method.getName()), protoKey);
    }

    public static MethodKey create(TypeKey declaring, StringKey name, Key protoKey) {
        return MethodKey.create(declaring, name, (ProtoKey) protoKey);
    }
    ...
    public static MethodKey create(TypeKey declaring, String name, Key protoKey) {
        return create(declaring, StringKey.create(name), (ProtoKey) protoKey);
    }
