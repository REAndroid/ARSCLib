...
public abstract class BlockArray<T extends Block> extends BlockContainer<T>
    ...
    public Collection<T> listItems(boolean skipNullBlocks){
        ...
        return new AbstractCollection<T>() {
            ...
            @Override
            public int size() {
                return BlockArray.this.getChildrenCount();
            }
        };
    }
    @Override
    ...
    public void ensureSize(int size){
        if(size <= getChildrenCount()){
            return;
        }
        setChildrenCount(size);
    }
    public void setChildrenCount(int count){
        ...
        int diff = count - getChildrenCount();
        if(diff==0){
            return;
        }
        changeSize(diff);
    ...
    public void insertItem(int index, T item){
        ...
        int lastIndex = children.length - 2;
        for(int i = lastIndex; i >= index; i--){
            T exist = children[i];
            children[i] = null;
            int newIndex = i + 1;
            children[newIndex] = exist;
    ...
    public final T getLast(){
        return get(getChildrenCount() - mFreeSpace - 1);
    }
    ...
    private class BlockIterator implements Iterator<T> {
        ...
        BlockIterator(boolean skipNullBlock){
            ...
            mCursor=0;
            mMaxSize=BlockArray.this.getChildrenCount();
        }
        @Override
